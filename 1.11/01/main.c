 ...
 * 
 * Stops reading on newline or EOF, and truncates input if it exceeds capacity.
 * 
 ...
 *          Always assign the return value back to your buffer variable.
 */
static inline char *ReallocateBuffer(char *buffer, size_t capacity) {
 ...
 * 
 * @warning Modifies the contents of @p buffer and @p index.
 */
static inline void CatChar(char *buffer, char ch, size_t *index) {
 ...
 *          After calling this function, the original pointer of @p buffer becomes NULL.
 */
void ExitWithError(char *buffer, const char *prompt) {
